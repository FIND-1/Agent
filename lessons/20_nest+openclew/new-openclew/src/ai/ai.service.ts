...
/**
 * 复习重点：
 * AiService 是用户请求侧的 agent loop，负责让模型选择 tool、执行 tool，再把 ToolMessage 放回上下文。
 *
 * 原文分析结论：
 * 创建定时任务时，本轮只调用 cron_job 保存任务；下半部分强调真正的邮件、搜索或数据库动作应留给未来触发时的 JobAgent。
 *
 * 依赖条件：
 * 流式接口里一旦检测到 tool_call chunk，就先停止向客户端透出中间文本，等工具执行后再进入下一轮。
 */
@Injectable()
export class AiService {
  ...
  constructor(
    ...
    @Inject('QUERY_USER_TOOL') private readonly queryUserTool: AppTool,
    ...
    @Inject('CRON_JOB_TOOL') private readonly cronJobTool: AppTool,
  ) {
    ...
    while (true) {
      ...
      for (const toolCall of toolCalls) {
        ...
        if (toolName === 'query_user') {
          const result = await invokeAppTool(this.queryUserTool, toolCall.args);
          ...
          const result = await invokeAppTool(this.cronJobTool, toolCall.args);
