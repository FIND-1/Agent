...
/**
 * 复习重点：
 * JobAgentService 是下半部分补上的后台 agent loop，用定时任务保存的 instruction 再决定调用哪些业务工具。
 *
 * 原文分析结论：
 * 后台执行器故意不绑定 cron_job，避免一个定时任务执行时继续创建新的定时任务，造成递归式自动化。
 *
 * 依赖条件：
 * 当前类已经具备执行能力，但 JobService 的运行时回调还没有真正调用 runJob(instruction)。
 */
@Injectable()
