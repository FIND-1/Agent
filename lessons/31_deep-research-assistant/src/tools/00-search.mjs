// 统一从项目根目录 .env 加载环境变量（lesson 31 使用 BOCHA_API_KEY、模型与 LangSmith 配置等）
...
// 复习定位：工具层只负责参数校验、Bocha 请求和结果格式化；researcher 子 Agent 决定何时调用它。
...
// —— 直接运行本文件时的演示入口：node src/tools/00-search.mjs [搜索关键词] ——
const isMain =
  Boolean(process.argv[1]) &&
  import.meta.url === pathToFileURL(process.argv[1]).href;

if (isMain) {
  const query = process.argv[2] ?? "2026 AI Agent 框架对比";
  console.log(`= 演示 web_search 工具 =`);
  const result = await webSearch.invoke({ query, count: 3 });
  console.log(result);
}
