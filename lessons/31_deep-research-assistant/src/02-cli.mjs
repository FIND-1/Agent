...
// 复习定位：CLI 只负责输入、流式事件格式化和产物列表展示，Agent 编排逻辑仍在 agent.mjs。
...
async function run(query) {
  ...
  console.log(`recursionLimit: ${agentConfig.recursionLimit}\n`);
...
async function main() {
  ...
  try {
    ...
    if (msg.includes("Recursion limit")) {
      console.error(
        `\n❌ recursion limit (${agentConfig.recursionLimit}) — set RECURSION_LIMIT in .env`,
      );
    } else {
      console.error("\n❌", err);
    }
