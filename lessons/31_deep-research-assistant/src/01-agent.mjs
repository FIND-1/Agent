...
import { webSearch } from "./tools/00-search.mjs";
...
// 复习定位：这是课程主线入口，用 createDeepAgent 把 skills、文件系统、todo、子 Agent
// 和 QuickJS 分析能力组合起来；实际调研仍依赖模型 API、Bocha API 与根目录 .env。

export const agentConfig = Object.freeze({
  recursionLimit: Number(process.env.RECURSION_LIMIT) || 300,
});
...
export function createIntelligenceDeskAgent(options = {}, temperature = 0) {
  ...
  const baseURL =
    options.baseURL ??
    process.env.DEEP_RESEARCH_MODEL_BASE_URL?.trim() ??
...
// —— 直接运行本文件时的演示入口：node src/01-agent.mjs [问题] ——
const isMain =
...
if (isMain) {
  ...
  const question = process.argv[2] ?? "请用一句话介绍你自己";
  console.log(`= 演示深度调研 Agent =`);
  console.log(`问题: ${question}\n`);
  const result = await agent.invoke(
    { messages: [{ role: "user", content: question }] },
    { recursionLimit: agentConfig.recursionLimit },
  );
