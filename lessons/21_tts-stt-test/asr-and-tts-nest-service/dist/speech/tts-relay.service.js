...
let TtsRelayService = TtsRelayService_1 = class TtsRelayService {
    ...
    constructor(configService) {
        ...
        this.voiceType = Number(configService.get('TTS_VOICE_TYPE') ?? 502006);
    }
    onModuleDestroy() {
    ...
    handleAiStreamEvent(event) {
        ...
        switch (event.type) {
            ...
            case 'end': {
                ...
                this.completeTencentStream(session);
                break;
            }
            case 'error': {
                this.sendClientJson(session.clientWs, {
    ...
    completeTencentStream(session) {
        if (!session.completionRequested ||
            session.pendingChunks.length > 0 ||
            !session.ready ||
            !session.tencentWs ||
            session.tencentWs.readyState !== ws_1.default.OPEN) {
            return;
        }
