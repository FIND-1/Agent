/**
 * 03：将云端 SDK 换成自托管 REST API，理解参数映射和 HTTP 错误。
 * 外部前置条件 / TODO：用户提供可用 Mem0 服务与数据库后才能运行。
 * 本例只封装文章用到的接口，不是完整 SDK；未验证自托管链路。
 */
import "@lessons/shared/env-loader";

const BASE_URL = process.env.MEM0_LOCAL_BASE_URL ?? "http://localhost:8888";
