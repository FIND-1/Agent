/**
 * Mem0 基础 CRUD 示例
 * 00：先认识云端 API；依赖 MEM0_API_KEY 和网络，无本地数据库要求。
 *
 * 学习重点：
...
async function main() {
  ...
  // 原注释写 dotenv/config；更正：共享 env-loader 按文件位置定位根 .env。
  ...
  /* 写入样本与下方 add 一起手动启用，默认查询不创建未使用变量。
    const conversation = [
  ...
  if (process.argv.includes("--cleanup")) {
    ...
    console.log(
      "\n提示: 运行 `node src/00-mem0-test.mjs --cleanup` 可删除本次测试用户的全部记忆",
    );
  }
}
