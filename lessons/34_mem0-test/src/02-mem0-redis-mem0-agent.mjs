 ...
 * 02：在 scope 示例上增加短期历史、摘要和结构化分类写入。
 * 外部前置条件 / TODO：Redis、Mem0 云端、模型 API；本次仅静态验证。
 * 原文的 Docker 启动命令仅作外部准备参考，不能据此假设服务可用。
 * 注意：当前 user_id 宽过滤不是严格分层；跨会话隔离仍待实现。
 *
...
import { z } from "zod";
import { createMem0Client } from "@lessons/shared/mem0-client";
...
class Mem0MemoryStore {
  ...
  // 保留文章的两次检索：user 查询可能覆盖 session 查询，结果也可能重复。
  async search(query) {
  ...
  // 布尔分类决定整轮写到哪里，并没有把长期事实与临时任务拆成不同文本。
  ...
  // userId 删除覆盖该用户范围，可能包含所有会话；不是只删当前 run。
  async clear() {
...
try {
  ...
  console.error(
    "Redis 未连接：请先确认外部 Redis 环境及根 .env 配置，Docker 部署步骤仍为 TODO。",
  );
  redis.disconnect();
  process.exit(1);
}
...
// 普通回答和分类器复用仓库模型工厂，分类器通过 schema 返回两个布尔决策。
const model = createChatModel({}, 0);
const classifier = createChatModel({}, 0).withStructuredOutput(memorySchema);
...
try {
  prevCount = (await redisStore.loadMessages(SESSION_ID)).length;
  while (true) {
 ...
 * 更正：仅重启进程不会清 Redis；需更换 MEMORY_SESSION_ID 或显式 :clear。
 * 当前 user_id 宽查询还可能带出旧会话任务，不能据此证明严格隔离。
 *
 * 六、聊多了会压缩（可选，连聊 8 轮以上）
 * 继续完善 Q1 总结 / 把第二段改短 / 加个标题……
 * → 终端会出现「已触发压缩」，老消息变摘要。
 * 更正：阈值是 8 条消息而非 8 轮；数量减少只是压缩线索。
 *
