/**
 * Mem0 三种记忆作用域示例
 * 01：相比基础 API，新增 user / run / agent 过滤维度；依赖云端 Key。
 * 更正：仅过滤 user_id 可能同时命中带 run_id 的记录，不等于纯用户层隔离。
 *
 * 这个文件用于验证三类记忆如何隔离存取：
 ...
 *
 * 推荐运行顺序：
 * 1. node src/01-mem0-scoped-memory-test.mjs add
 * 2. 等待几秒，让 Mem0 完成异步记忆处理。
 * 3. node src/01-mem0-scoped-memory-test.mjs search
 * 4. node src/01-mem0-scoped-memory-test.mjs --cleanup
 *
 * 注意：
...
async function searchUserMemory(client) {
  // filter 字段使用 snake_case；原注释称只检索用户层。
  // 更正：user_id 是用户范围，可能包含该用户其他会话的记忆。
...
// 统一格式化输出，便于比较 API 返回结构；简单日志直接保留在示例内。
