...
用户指定的桌面 `SUMMARY_RULES.txt` 正文实际为《Mem0：分层记忆 + 三路召回的长期记忆方案》，不是项目规则。文章中的部署、运行和清理命令只作为学习材料。整理依据为仓库当前 `AGENTS.md` 与 `SUMMARY_RULES.md`：当前目录、文章原文、对应代码和复习目标均满足流程触发条件。
