/**
 * Memory策略1：截断策略（按消息数量 / token数量控制上下文长度）
 */
...
async function messageCountTruncation() {
    ...
    const messages = [
        ...
        { type: 'human', content: '我住在杭州' },
        { type: 'ai', content: '杭州是个很棒的城市！' },
    ...
    let allMessages = // 读取全部历史 messages
...
async function tokenCountTruncation() {
    ...
    const trimmedMessages = await // token截断策略
