// 承接 00 的签名教学：签发绑定 key 的 Post Policy，让浏览器直接上传原图。
// 生成结果则由后端下载再 put 到 OSS；签名上传权限与对象读取权限需要分开理解。
import {
  Injectable,
...
export class OssService {
  ...
  createUploadPolicy(ext = '.jpg'): OssUploadSignature {
    // 这里不是 STS AssumeRole，没有临时 SecurityToken；Secret 始终留在服务端。
  ...
  getSignedUrl(objectKey: string, expires = 3600): string {
    // 私有输入图供模型短时读取；列表中的普通 URL 不会因此自动变成签名 URL。
  ...
  private wrapOssError(error: unknown, action: string): Error {
    // SDK 通常抛出 Error；同时处理字符串、null 等异常值，避免错误处理再次抛错。
    const code =
      error &&
      typeof error === 'object' &&
      'code' in error &&
    ...
    this.logger.error(
      `OSS ${action} failed: ${code ?? 'UnknownError'} - ${message}`,
    );
    ...
    return new InternalServerErrorException(`OSS ${action} failed: ${message}`);
  }
}
