...
- 原文主线：大模型只能「说」→ 用 tool 让它「做」→ 获取 api key → `src/hello-langchain.mjs` 调通模型 → 改用 `.env` 管理密钥 → `src/tool-file-read.mjs` 定义 read_file → `bindTools` → 观察 `tool_calls` → 应用侧执行工具 → `ToolMessage` 回填 → 模型给出最终解释。
...
3. 图片归档：原文 14 张配图按正文出现顺序保存到 `assets/`，并在 README 建立「图 → 正文位置 → 知识点」索引。
4. 文档补齐：新增 `README.md`（学习路径、运行方式分组、关键结论、常见报错）与 `REVIEW_NOTES.md`。
...
README.md
REVIEW_NOTES.md
assets/01-agent-tool-ability.png ... assets/14-final-reply-output.png
...
- 课程根目录只有 `README.md` 与 `REVIEW_NOTES.md` 两份 Markdown。
...
3. **循环写法**：原文用 `while` + `Promise.all` 并发执行工具；本课文件用 `while (true)` + `for` 顺序执行，便于逐条观察入参与结果。复习重点仍是循环与 `tool_call_id` 关联，并发写法差异已列入 README 待办。
...
- 结果：原文 14 张嵌入图片已全部提取并按正文顺序保存到 `assets/`，命名带两位序号，README 第 8 节给出「图 → 正文位置 → 知识点」索引。
- 限制：本次执行环境的模型不支持图像输入，**未对图片做 OCR / 视觉识别**。图注是依据图片在 `word/document.xml` 中的段落位置与相邻正文推断的，不是从图片内容读出的。
- 影响范围：如果某段代码只出现在图片里，则未被转录。实际核对结果是：正文本身已给出 tool 定义、`bindTools`、工具循环、`ToolMessage` 回填这几段代码文本，因此**代码链路是完整的**，图片主要承担控制台截图与运行输出记录的作用。
- 如需按图复习，请直接打开 `assets/` 下对应文件；若发现某张图包含正文没有的代码，请补充说明，我再按图对齐代码与文档。
