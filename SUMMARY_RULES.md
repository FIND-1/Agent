...
1. 如果同一类公共方法只在单个 lesson 内使用，或只在 2-3 个 lesson 中复用，可以继续保留在对应 lesson 的 `src/_shared/`。
2. 如果同一类公共方法已经在 **超过 3 个 lesson** 中重复出现，必须提取到 `lessons/_shared/`，并更新所有相关引用。
